...
```{r}
# Insert code below
# I directly imported the "control_patients.csv" and "case_patients.csv"to my RStudio by clicking "Import Dataset -> From Text(base)..."(in the Environment box of the top right corner section).
d.in1 <- read.csv("~/Desktop/Assignment 2 Datasets/control_patients.csv", header = TRUE)
d.in2 <- read.csv("~/Desktop/Assignment 2 Datasets/case_patients.csv", header = TRUE)
d.in1 <- d.in1 %>%
...
boxplot(los~outcome, data = d.in, xlab = "outcome", ylab = "los", ylim=c(0,135), main = 'Los versus Outcomes')

# Observations: The control_patients boxplot has more outliers in the above maximum. The case_patients boxplot has a larger range for the third quartile than the control_patients boxplot. This means the length of stays for control_patients group are more concentrated around the median LOS than the data distribution in the case_patients file.
```
...
```{r}
# Insert code below
d.box1 <- boxplot(los~gender, data = d.in1, xlab = "Gender", ylab = "los", ylim=c(0,135), main = 'Control patients Los versus genders')
```
```{r}
d.box2 <- boxplot(los~gender, data = d.in2, xlab = "Gender", ylab = "los", ylim=c(0,125), main = 'Case patients Los versus Gender')

#Observations: The case_patients file has a larger range for the third quartile than the control_patients regardless of the females or males data sets. The data in case_patients file are more concentrated around the medium Length of Stay (LOS) than the control_patients file data. The control_patients data sets have more outliers above the maximum. The first quartile range of control_patients data is very tiny. The case_patients file has a larger first quartile range than the control_patients datasets.
```
...
library(formattable)
d.in3 <- read.csv("~/Desktop/Assignment 2 Datasets//smokedata.csv", header = TRUE)

d.in3 <- d.in3 %>% 
  filter(quit == '0' | quit == '1') %>%
  filter(age_at_appt <= 100) %>%
  filter(age_at_appt > 0) %>%
  droplevels()
...
ill.table
percent((131+34)/(d.total))
percent((18+13)/(d.total))
...
smoke.table
percent((127+22)/(d.total))
percent((22+25)/(d.total))
chisq.test(smoke.table)

provider.table <- with(d.in4, table(quit,provtype))
provider.table
percent((135+43)/(d.total))
percent((14+4)/(d.total))
chisq.test(provider.table)
