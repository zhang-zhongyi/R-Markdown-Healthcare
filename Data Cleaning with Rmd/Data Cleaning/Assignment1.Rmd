...
Ans 1.  
The answers are a) -0.0001 and c)3.415. Probability canot be negative numbers since they will be meaningless. Probability cannot over 1. The highest probability is 100%. 341.5% is meaningless.
...
Ans 2. 
The probability that I will win the game is 4/13 if the jokers cards are taken out from the ordinary playing cards.52 cards will be the total amount of cards. 4 aces include 1 ace with spade. There are 13 spades in total. (13+4-1)/52 = 4/13 is the probability that I will win the game. 
...
a) P(exactly 7 survive) = C(17,7) * 0.8^7 * 0.2^(17-7) = 19448 * 0.2097152 * 1.024 * 10^(-7) = 0.0004176426199 = 0.041764%

The chance that exactly 7 will survive is 0.041764%.

b) P(exactly 15 survive) = C(17,15) * 0.8^15 * 0.2^(17-15) = 0.19140298416
   P(exactly 16 survive) = C(17,16) * 0.8^16 * 0.2^(17-16) = 0.09570149208
   P(exactly 17 survive) = C(17,17) * 0.8^17 * 0.2^(17-17) = 0.02251799814
...
--------------|-----------------------|----------------------|------------------
Age > 70 (%)  |        453       (2%) |      4,728      (20%)|  5,181    (21%)
Age <= 70 (%) |      1,672       (7%) |     17,254      (72%)| 18,926    (79%)
Total         |      2,125       (9%) |     21,982      (91%)| 24,107   (100%)
...
#Calculate the total amount of patients
d.total <- d.in %>%
  select(patient.ids) %>%
  unique() %>%
  nrow()
...
#Calculate the total amount of patients who are elder than 70
d.elderthan70 <- d.in %>%
  ...
  nrow()
d.elderthan70
dp1 <- d.elderthan70/d.total
percent(dp1)


#Calculate the total amount of patients who are younger than or equal to 70
d.youngerthan70 <- d.in %>%
  filter(age_at_admit <= 70) %>%
  select(patient.ids) %>%
  unique() %>%
  nrow()
d.youngerthan70
dp2 <- d.youngerthan70/d.total
percent(dp2)

#Calculate the total amount of patients who had cardiac arrests
d.yes <- d.in %>%
...
#Calculate the total amount of patients who didn't have cardiac arrests
d.no <- d.in %>%
...
#Calculate the total amount of patients who are elder than 70 and had cardiac arrests
d.yes_elderthan70 <- d.in %>%
  filter(age_at_admit > 70, had_cardiac_arrests == 1) %>%
  select(patient.ids) %>%
  unique() %>%
  nrow()
d.yes_elderthan70
dp5 <- d.yes_elderthan70/d.total
percent(dp5)

#Calculate the total amount of patients who are younger than or equal to 70 and had cardiac arrests
d.yes_youngerthan70 <- d.in %>%
  filter(age_at_admit <= 70, had_cardiac_arrests == 1) %>%
  select(patient.ids) %>%
  unique() %>%
  nrow()
d.yes_youngerthan70
dp6 <- d.yes_youngerthan70/d.total
percent(dp6)

#Calculate the total amount of patients who are elder than 70 and didn't have cardiac arrests
d.no_elderthan70 <- d.in %>%
...
#Calculate the total amount of patients who are younger than or equal to 70 and didn't have cardiac arrests
d.no_youngerthan70 <- d.in %>%
  filter(age_at_admit <= 70, had_cardiac_arrests == 0) %>%
  select(patient.ids) %>%
  unique() %>%
  nrow()
d.no_youngerthan70
dp8 <- d.no_youngerthan70/d.total
percent(dp8)

#Just take an overview
percent(list(dp1, dp2, dp3, dp4, dp5, dp6, dp7, dp8))
```
...
# I directly imported the "patient_dataset.csv" to my RStudio by clicking "Import Dataset -> From Text(base)..."(in the Environment box of the top right corner section).
...
d.in <- d.in %>%
  mutate(hosp_admission_deid = hosp_admission_form + ddays(random_shift),
         hosp_discharge_deid = hosp_discharge_form+ ddays(random_shift))
...
d.in <- mutate(d.in, length_of_stay = interval(hosp_admission_form, hosp_discharge_form) / ddays(1))

d.in <- d.in %>%
  select(-c(hosp_admission, hosp_discharge,
            hosp_admission_form, hosp_discharge_form,
            dob, dob_form, temp_interval, hosp_admission_deid, hosp_discharge_deid))

d.in <- d.in %>%
  select(-c(admitting_provider))

write.csv(d.in, "patient_dataset_modified.csv", row.names = FALSE, quote = TRUE)
